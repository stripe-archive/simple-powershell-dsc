...
func (w *ResponseWriter) End() events.APIGatewayProxyResponse {
	// NOTE(andrew-d): we always want to base64-encode, since our API
	// Gateway will probably not know how to deal with proxying each
	// individual path distinctly; this lets us set CONVERT_TO_BINARY on
	// the entire API.
	w.out.IsBase64Encoded = true
